...
class _PendingAttendanceState extends State<PendingAttendance> {
  ...
  // Current view state
  String _currentView = 'initial'; // 'initial', 'project', 'zone', 'ward', 'attendance'
  ...
  Future<void> _checkLoginStatus() async {
    ...
    await prefs.setString("menu", "pending");
  ...
  Future<void> _fetchSupervisorAttendance() async {
    try {
      ...
      final response = await http.get(
        Uri.parse("$baseUrl/hr/drf_unmarked_list/?attendance_date=$formattedDate"),
        headers: {
      ...
      print('Supervisor API: ${"$baseUrl/hr/drf_unmarked_list/?attendance_date=$formattedDate"}');
  ...
  Future<void> _fetchProjects() async {
    try {
      final auth = 'Basic ${base64Encode(utf8.encode('$username:$password'))}';
      final response = await http.get(
        Uri.parse("$baseUrl/hr/drf_unmarked_dash/"),
        headers: {
          'Content-Type': 'application/json',
          'authorization': auth,
        },
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          // Clear child data when going to projects
          _currentZone = '';
          _currentWard = '';
          _currentZoneId = 0;
          _currentWardId = 0;
  ...
  Future<void> _fetchProjectZones(String project) async {
    ...
    try {
      final auth = 'Basic ${base64Encode(utf8.encode('$username:$password'))}';
      final response = await http.get(
        Uri.parse("$baseUrl/hr/drf_unmarked_dash_project/?project=$project&attendance_date=$formattedDate"),
        headers: {
          'Content-Type': 'application/json',
          'authorization': auth,
        },
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          // Clear child data when going to zones
          _currentZone = '';
          _currentWard = '';
          _currentZoneId = 0;
          _currentWardId = 0;
  ...
  Future<void> _fetchZoneWards(int zoneId, String zoneName) async {
    ...
    try {
      final auth = 'Basic ${base64Encode(utf8.encode('$username:$password'))}';
      final response = await http.get(
        Uri.parse("$baseUrl/hr/drf_unmarked_dash_zone/?zone_id=$zoneId&attendance_date=$formattedDate"),
        headers: {
          'Content-Type': 'application/json',
          'authorization': auth,
        },
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          // Clear child data when going to wards
          _currentWard = '';
          _currentWardId = 0;
        });
      } else {
  ...
  Future<void> _fetchWardAttendance(int wardId, String wardName) async {
    ...
    try {
      final auth = 'Basic ${base64Encode(utf8.encode('$username:$password'))}';
      final response = await http.get(
        Uri.parse("$baseUrl/hr/drf_unmarked_dash_ward/?ward_id=$wardId&attendance_date=$formattedDate"),
        headers: {
          'Content-Type': 'application/json',
          'authorization': auth,
        },
  ...
  void _navigateBack() {
    if (_navigationStack.isNotEmpty) {
      ...
      // If no navigation stack, go to initial data
      _loadInitialData();
    }
  }

  bool _shouldShowBackButton() {
    // For Supervisor/Vehicle Incharge, show back button only when in selection mode or searching
    if (usertype == 'Supervisor' || usertype == 'Vehicle Incharge'||_currentView == 'project') {
      return _selectionMode || _isSearching;
    }
    // For other users, show back button when not in initial view or when navigation stack is not empty
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          // Navigation track Breadcrumb navigation for non-supervisor users
        /*  if ((usertype != 'Supervisor' && usertype != 'Vehicle Incharge') &&
              _currentView != 'initial' &&
              !_isSearching &&
              !_selectionMode)
          ...
          Expanded(
            child: RefreshIndicator(
              onRefresh: _refreshCurrentData, // Refresh only current screen data
              child: isLoading
    ...
    switch (_currentView) {
      ...
      case 'attendance':
        return AttendanceListBuilder2(
          ...
          onAttendanceMarked: _refreshCurrentData, // Refresh current data only
          selectionMode: _selectionMode,
...
// ... (Keep the existing AttendanceListBuilder2 class exactly as it was in the previous code)
// The AttendanceListBuilder2 class remains unchanged from the previous implementation
