...
class _AttendancePageState extends State<AttendancePage> {
  ...
  Future<void> _checkLoginStatus() async {
    ...
    await prefs.setString("menu", "biometric_attendance");
...
class _AttendanceListBuilderState extends State<AttendanceListBuilder>
  ...
  Widget _buildSupervisorCard(dynamic item, BuildContext context) {
    final employeeId = item['employee_id']?.toString() ?? 'N/A';
    final employeeCode = item['essl_code']?.toString() ?? 'N/A';
    final employeeName = item['employee_name']?.toString() ?? 'N/A';
    final employeeWard = item['ward']?.toString() ?? 'N/A';
    final RoundColor = item['round_dot_color']?.toString() ?? 'N/A';
    final Designation = item['designation']?.toString() ?? 'N/A';
    final color = item['color_code']?.toString() ?? 'N/A';
    final essl = item['essl']?.toString() ?? 'N/A';
    final manual = item['manual']?.toString() ?? 'N/A';
    final esslStatus = item['essl_status'] ?? false;
    final esslIntime = item['essl_intime'] ?? 'N/A';
              ...
              child: Slidable(
                ...
                child: ListTile(
                  ...
                  subtitle: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('${employeeId}_$employeeName'),
                      Text(
                        employeeWard,
                        style: const TextStyle(
                          fontWeight: FontWeight.bold,
                      ...
                      Text('ESSL Status: $essl'),
                      esslIntime != ''
                          ? Text('ESSL Intime: $esslIntime')
                          : Container(),
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          Text('Manual Status: $manual'),
                          RoundColor != null && RoundColor != ''
                              ? Icon(Icons.circle,
                                  color: _hexToColor(RoundColor), size: 25)
                              : Container()
                        ],
                      ),
